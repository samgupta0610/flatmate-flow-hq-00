  ...
  return <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md p-0 gap-0">
        <div className="relative">
          {/* Close button */}
          <Button variant="ghost" size="sm" onClick={skipOnboarding} className="absolute top-4 right-4 z-10 text-muted-foreground hover:text-foreground">
            <X className="w-4 h-4" />
          </Button>
          ...
          <Card className="border-0 shadow-none">
            <CardContent className="p-8 text-center space-y-6">
              ...
              <div className="flex justify-center gap-2 mb-6">
                {steps.map((_, index) => <div key={index} className={`w-2 h-2 rounded-full transition-all duration-300 ${index <= step ? 'bg-primary' : 'bg-muted-foreground/20'}`} />)}
              </div>
              ...
              <div className="space-y-4">
                ...
                {steps[step].highlight && <div className="bg-gradient-to-r from-primary/10 to-accent/10 p-3 rounded-lg border border-primary/20">
              ...
              <div className="flex justify-between items-center pt-6">
                <Button variant="ghost" onClick={skipOnboarding} className="text-muted-foreground hover:text-foreground">
                  Skip tour
                </Button>

                <Button onClick={nextStep} className="bg-gradient-primary hover:shadow-glow flex items-center gap-2 text-slate-950">
                  {step === steps.length - 1 ? <>
                      Get Started
